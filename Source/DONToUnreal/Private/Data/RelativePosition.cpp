...
/**
 * Handles when one position is more nested than the other. Adds its position to its parent's position to its parent's position all the way up the tree until we have a coordinate rooted on the same parent as the other.
 *
 * There is a chance that LessNested and MoreNested are actually on different branches of the tree, meaning even with matching NestIndexes they don't have the same parent.
 * This case redirects to @link GetPositionsRelative_Both @endlink .
 * @param LessNested 
 * @param MoreNested 
 * @param OutLessNested 
 * @param OutMoreNested 
...
/**
 * Handles the case where both are on the same NestIndex, but they still have different parents.
 *
 * NOTE: ADDS to OutPos1 and OutPos2 rather than overwriting them so it can be combined with @link GetPositionsRelative_Higher @endlink .
 * @param Pos1 
 * @param Pos2 
 * @param OutPos1 
 * @param OutPos2 
